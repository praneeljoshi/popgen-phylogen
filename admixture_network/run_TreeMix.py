...
def run_TreeMix(file, admixture_count, outgroup=None, snp_group_size=None, output="out_stem", treemix_prefix="treemix"):
    """
    Runs GTmix on an input file, and returns the network that it produces.
    ...
    # Parse output graph
    with gzip.open(output + '.vertices.gz', 'rt') as vertices_file, gzip.open(output + '.edges.gz', 'rt') as edges_file:
    #with gzip.open('out_stem.vertices.gz', 'rt') as vertices_file, gzip.open('out_stem.edges.gz', 'rt') as edges_file:
        # Initalize network
        network = nx.DiGraph()
