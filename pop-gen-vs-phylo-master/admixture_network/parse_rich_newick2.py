...
def merge_admixture_using_tag(node, mix_tag, network):
    ...
    if mix_tag in mix_tags: # Other node exists
        ...
        # Retain proportion information if only one of the nodes has it
        if network.nodes[node]["proportion"] is not None:
            merge_nodes(node, other_node, network)
        else:
            merge_nodes(other_node, node, network)
...
def parse_rich_newick(string, strip_comments=True):
    ...
    # Clean up counter and mix_tags attributes
    network.graph.pop("counter")
    network.graph.pop("mix_tags")

    return network

def modify_BirthDeath_str(string):
    """
    Modifies a network string generated by BEAST so that it can be read in as a Rich Newick format network by properly encoding admixture proportions.
    """
    return re.sub(r'\[&gamma=([:0-9.]*)[^\]]*\][:0-9.eE-]*', lambda match: match.group() + "::" + match.group(1), string)
...
if __name__ == "__main__":
    ...
    # test = "((((2:0.024304008779069148,5:0.024304008779069148)I2:0.01892788203415682,1:0.04323189081322597)I3:3.020971242141332E-4)I6#H1:6.5720255822085605::0.13021880332708125,(6:0.07257442236116766,((4:0.05281982098962986,3:0.05281982098962986)I1:0.013781798418543996,I6#H1:0.023067631470733756::0.8697811966729188)I0:0.005972802952993808)I4:6.542985147784833)I5;"
    test = "((6:0.07257442236116766,((4:0.05281982098962986,3:0.05281982098962986)I10:0.013781798418543996,(((2:0.024304008779069148,5:0.024304008779069148)I3:0.01892788203415682,1:0.04323189081322597)I2:0.0003020971242141332)I13#H13:0.023067631470733756::0.8697811966729188)I9:0.005972802952993808)I7:6.542985147784833,I13#H13:6.5720255822085605::0.13021880332708125)I0;"

    test_network = parse_rich_newick(test)

    test_viz = nx.nx_pydot.to_pydot(test_network)
    print(test_viz)
    test_viz.write_pdf("test3.pdf")
