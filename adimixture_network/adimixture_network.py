...
# Adimixture nodes --> indegree = 2, outdegree = 1
...
def generate_topology_only_extant(pop_count, adimixture_count):
    """
    Generates the topology of an adimixture network where only extant populations are adimixed.
    """
    network = generate_random_fbt(pop_count + adimixture_count)
    ...
    while True:
        combined_leaves = random.sample(leaves, 2 * adimixture_count)
        no_siblings = True
    ...
    # Form adimixture nodes
    for i in range(0, len(combined_leaves), 2):
        # Merge combined_leaves[i] and combined_leaves[i+1] into combined_leaves[i], and make it an adimixture node
        network.add_edge(next(network.predecessors(combined_leaves[i+1])), combined_leaves[i])
        network.nodes[combined_leaves[i]]["type"] = "adimixture"
...
def generate_topologies_any(pop_count, adimixture_count, n):
    """
    Generates the topologies of n adimixture networks where any populations can be adimixed.
    """
    internal_count = adimixture_count + pop_count - 2 # Root node is not an internal node for counting purposes (-1 -1)
    total_count = adimixture_count + pop_count + internal_count + 1
    ...
    for node, attributes in base_network.nodes.items():
        ...
        elif internal_count < node <= adimixture_count + internal_count: # Adimixture node
            base_network.nodes[node]["type"] = "adimixture"
        else: # Leaf node
            base_network.nodes[node]["type"] = "leaf"
            base_eligible_u.remove(node)

    edge_count = (2 + 3 * (internal_count + adimixture_count) + pop_count) // 2 # Handshaking lemma
...
def add_outgroup_time_mix_tags(network, time_interval, outgroup_time_bonus, adimixture_prop):
    """
    Takes in the topology of an adimixture network, and adds an outgroup as well as time and adimixture proportion tags.
    Modifies the network in place as well as returning it.
    """
    # Scan for leaves and old root, and also add in adimixture proportions
    leaves = set()
    old_root = 0
    ...
    for node, attributes in network.nodes.items():
        if attributes["type"] == "leaf":
            ...
            old_root = node
        elif attributes["type"] == "adimixture":
            attributes["proportion"] = adimixture_prop
    ...
    # Assign times to each internal/adimixture node
...
def generate_ms_command(network, alleles_per_pop, loci_count, mutation, recombination, locus_length):
    """
    Generates a command that uses the ms software package to generate random haplotypes based on a given adimixture network
    ...
    cmd = f"ms {pop_count * alleles_per_pop} {loci_count} -t {mutation} -r {recombination} {locus_length} -I {pop_count} {(str(alleles_per_pop) + ' ') * pop_count}"
    ...
    for focus_node in time_order:
        ...
        lower_edges = list([x for x in network.edges.data() if x[0] == focus_node]) # Is there a better way?
        higher_edges = list([x for x in network.edges.data() if x[1] == focus_node])
        if focus_type in ["internal", "root"]: # Merge the two lower edge populations into one higher edge population
            ...
            if focus_type != "root": # Special case for tree root
                higher_edges[0][2]['population'] = lower_edges[1][2]['population'] # must be this population, not the other one
        elif focus_type == "adimixture": # Split the one lower edge population into two higher edge populations
            cmd += f"-es {network.nodes[focus_node]['time']} {lower_edges[0][2]['population']} {network.nodes[focus_node]['proportion']} "
            higher_edges[0][2]['population'] = lower_edges[0][2]['population']
            higher_edges[1][2]['population'] = pop_count + new_pop_counter
            new_pop_counter += 1

    return cmd

def generate_adimixture_networks(pop_count, adimixture_count, time_interval, outgroup_time_bonus, adimixture_prop, alleles_per_pop, loci_count, mutation, recombination, locus_length, only_extant_adimixture, n):
    """
    Generates n random adimixture networks according to the provided parameters. 
    Returns a list of tuples, where the first element in each tuple is an adimixture network, and the second element is a command for ms.
    """
    if only_extant_adimixture:
        networks = [generate_topology_only_extant(pop_count, adimixture_count) for _ in range(n)]
    else:
        networks = generate_topologies_any(pop_count, adimixture_count, n)
    networks_modified = [add_outgroup_time_mix_tags(network, time_interval, outgroup_time_bonus, adimixture_prop) for network in networks]
    return list([(network, generate_ms_command(network, alleles_per_pop, loci_count, mutation, recombination, locus_length)) for network in networks_modified])

if __name__ == "__main__":
    test_results = generate_adimixture_networks(4, 1, 0.02, 0.3, 0.5, 4, 2, 50, 50, 500000, True, 1)
